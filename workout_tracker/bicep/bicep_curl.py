...
def run_bicep_session():
    ...
    while True:
        ...
        # -------- REP COUNTING --------
        # -------- OPTIMIZED REP COUNTING --------

        upper_threshold = 90  # Arm extended
        lower_threshold = 30  # Arm contracted

        # Right Arm
        if right_range >= upper_threshold:
            right_state = "down"

        if right_range <= lower_threshold and right_state == "down":
            right_state = "up"
            if current_msg == "":
                rcount += 1

        # Left Arm
        if left_range >= upper_threshold:
            left_state = "down"

        if left_range <= lower_threshold and left_state == "down":
            left_state = "up"
            if current_msg == "":
                lcount += 1
